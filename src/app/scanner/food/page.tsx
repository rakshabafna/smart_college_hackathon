...
export default function FoodScannerPage() {
  ...
  const handleRedeem = () => {
    ...
    // Check if this code has already been redeemed for THIS hackathon
    ...
    if (alreadyUsed) {
      ...
      const log = { id: `scan-${Date.now()}`, studentId: "demo", studentName: "Student", hackathonId: hackId, type: mealType, result: "already_used" as const, timestamp: new Date().toISOString() };
      ...
      const log = { id: `scan-${Date.now()}`, studentId: student?.id ?? "demo", studentName: name, hackathonId: hackId, type: mealType, result: "valid" as const, timestamp: new Date().toISOString() };
      Store.addScanLog(log);
